...
const MAX_PARALLEL_POSTS = 1; // Максимальное количество параллельных отправок
...
async function processQueue() {
  ...
  // Создаем массив для отслеживания активных отправок
  const activePosts = new Set();
  
  while (postQueue.length > 0 || activePosts.size > 0) {
    // Запускаем новые отправки, если есть свободные слоты
    while (postQueue.length > 0 && activePosts.size < MAX_PARALLEL_POSTS) {
      const post = postQueue.shift();
      currentPost++;
      
      const postPromise = sendPost(post, currentPost, totalPosts)
        .finally(() => {
          activePosts.delete(postPromise);
        });
      
      activePosts.add(postPromise);
    }
    
    // Ждем завершения хотя бы одной отправки
    if (activePosts.size > 0) {
      await Promise.race(activePosts);
    }
  }
